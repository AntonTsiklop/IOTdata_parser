...
layout = [  [sg.InputText(key='-link-'), sg.Text('Ссылка')],
            [sg.InputText(key='-carid-'), sg.Text('Буи (id через пробел)')],
            [sg.InputText(key='-d_start-'), sg.Text('Начало (гггг-мм-дд чч:мм:сс)')],
            [sg.InputText(key='-d_stop-'), sg.Text('Конец (гггг-мм-дд чч:мм:сс)')],
            [sg.ProgressBar(max_value=100000, orientation='h', size=(30,15), key='-PROG-'), sg.Text('Обработка данных')],
            [sg.Button('Данные из полей ввода')],
            [sg.Button('Данные из .ini')]  ]
...
while True:
    ...
    if event == 'Данные из полей ввода':
        link = values['-link-']
        ...
        window['-carid-'].update(carid_str)
        window['-d_start-'].update(d_start_str)
        window['-d_stop-'].update(d_finish_str)
